/*
 * i2c_driver.c -> simple template driver
 *                                                                             
 * GPL                                                                      
 ...
 *
 * - create sysfs entry (/sys/devices/virtual/i2c_driver/i2c_driver)
 * - create /dev entry via udev (/etc/udev/rules/90-<modulename>.rules)
 *   entry -> KERNEL=="i2c_driver", MODE="0666"
 */
	/* get a device number */
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("simple i2c - template driver");
